...
import { fetchUsers, addBlankUser, loadUserRoles } from '../store/users/action';
import {
  handleUserCreation,
  handleUserDeletion,
  handleUserUpdate,
...
class DomainPage extends Component {
  ...
  componentWillMount() {
    ...
    dispatch(fetchUsers(this.state.currentdomainName)).then(() => {
      ...
      users.forEach(user => {
        ...
        dispatch(loadUserRoles(this.state.currentdomainName, user.id)).then(() => {
  ...
  handlePlusClick() {
    const { clients, roles, users } = this.state;
        ...
        {users.length !== 0 ? (
          users.map((user, i) => (
            <UserWidget
              ...
              handleRoleChange={(
                value, roleName, roleId, userIndex, checkBoxIndex
              ) =>
  ...
  onUserSave(index) {
    ...
    if (id !== undefined) {
      if (realmRoles.length >= 0) {
        this.props.dispatch(unAssignUserRoles(realm, id, this.state.roles)).then(() => {
